  ...
  run() {
    ...
    const questions = [
      {
        type: 'input',
        name: 'text',
        message: 'The text content of the logo must not be empty and it can be up to three characters long. Please enter the desired text for your logo: ',
      },
      {
        type: 'input',
        name: 'color',
        message: `To select a color, you must enter the SVG color keyword or the hexadecimal rgb color code. 
    ...
    return inquirer
      ...
      .then(({ text, color, shape, fill }) => {
         ...
         if(!isValidLogoText(text)){ 

          console.log('\u001b[31m\nInvalid text for the logo. Try again.\n\u001b[0m');
          this.run();
        
        }else if(!isValidSVGColorKeyword(color) && !isValidHexRGBColorCode(color)){
        
          console.log("\u001b[31m\nInvalid color for the logo's text. Try again.\n\u001b[0m");
          this.run();
  
        }else if(!isValidSVGColorKeyword(fill) && !isValidHexRGBColorCode(fill)){
